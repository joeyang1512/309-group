<template>
  <div>
    ...
    <div class="col-md-9" style="padding:20px 0 ;">
      <ul class="breadcrumb" style="background-image: linear-gradient(to bottom, rgba(190,220,240,1), rgba(255,250,250,0));">
        <li>309</li>
        <li>著作专利</li>
        <li>{{typeName[id-1]}}</li>
      </ul>
      ...
      <div class="col-md-4" style="text-align: center;font-size:14px" v-show="id===1"  v-for="(item,index) in indexInfo" :key="'index'+index">
        <!-- <div style="padding:0;margin:0" v-for="(item,index) in indexInfo" :key="'index'+index"> -->
          ...
          <tbody style="font-size: 10px">
            ...
            <tr v-for="(item,index) in patent" :key="'patent'+index" style="font-size:14px">
              <td>
                ...
                <router-link :to="{path:'/patentDetail/id',query:{id:item.id}}" ><a>{{item.name}}</a></router-link>
              </td>
              <td>{{item.origin}}</td>
              <td>{{item.data}}</td>
            </tr>
...
<script>
import { getWritingIndex ,getWritingChinapaper,getWritingEnglishpaper,getWritingPatent} from "@/api/api";
// @ is an alias to /src
export default {
  name: "writingDetail",
  props: ["id"],
